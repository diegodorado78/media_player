 export interface Ad{//define la estructura de Ad
    imageUrl: string;
    title: string;
    body: string;
    url: string;
}
//lista de add iniciales que siguen la estructura de Ad
const ALL_ADS: Ad[] = [
    {
        imageUrl:
...
class Ads {
    ...
    private ads: Ad[]// arreglo del tipo Ad definido en la interfaz
    
    private constructor() {
        this.iniAds();//inicializa construyendo los adds
    }

    static getInstance() {// los estaticos se ejecutan sin necesidad de una instancia
        if (!Ads.instance) {// checa si no hay una instancia y si no la crea
            Ads.instance = new Ads();
        }
        return Ads.instance;
    }
    //necesitamos una lista inicial de adds
    private iniAds() {//
        this.ads=[...ALL_ADS]//crear copia del arreglo sin dañarel otro
    }
    //metodo para devolver un add a quien lo pida
    getAd() {
        if (this.ads.length === 0) {
            this.initAds();//cuando sean cero los vuelve a inicializar
        }
        return this.ads.pop();//arreglo que tiene todos los ads
        //pop saca el ultimo valor y lo devuelve
    }
}
export default Ads;